...
jq -r "$JQ_QUERY" "$INDEX_FILE" | while IFS=$' ' read -r URL BASE_NAME; do
    ...
    if [[ -f "$VIDEO_OUTPUT" ]]; then
        ...
        # ADDED FLAGS for MAC/CHROME: --geo-bypass-country US and --cookies-from-browser chrome
        yt-dlp -f 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best' \
               ...
               "$URL"
        if [ $? -ne 0 ]; then
            echo "    [ERROR] Video download failed for $BASE_NAME. Check URL: $URL"
        fi
    fi

    # --- 2. Download Audio (MP3) ---
    AUDIO_OUTPUT="$AUDIO_DIR/$BASE_NAME.mp3"
    if [[ -f "$AUDIO_OUTPUT" ]]; then
        echo "  - Audio already exists. Skipping audio download."
    else
        echo "  - Downloading Audio to $AUDIO_OUTPUT"
        # ADDED FLAGS for MAC/CHROME: --geo-bypass-country US and --cookies-from-browser chrome
        yt-dlp -x --audio-format mp3 \
               ...
               "$URL"
        if [ $? -ne 0 ]; then
            echo "    [ERROR] Audio download failed for $BASE_NAME. Check URL: $URL"
        fi
    fi
    echo "----------------------------------------------------"
done

echo "✅ All downloads attempted. Check the '$DOWNLOADS_ROOT' directory."
